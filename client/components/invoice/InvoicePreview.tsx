...
const InvoicePreview = forwardRef<HTMLDivElement, Props>(({ data }, ref) => {
  ...
  return (
    <div
      ...
      style={{ 
        fontFamily: 'Inter, system-ui, -apple-system, Segoe UI, Roboto, Ubuntu, Cantarell, Noto Sans',
    ...
    >
      {/* Top brand bar with logo (fixed so it repeats visually on each generated PDF page via capture) */}
      ...
      <div className="px-10">
                ...
                <img
                  src={
                    data.status === "Rejected"
                      ? "/client/components/asserts/image1.png"
                      : data.status === "Completed"
                      ? "/client/components/asserts/image2.png"
                      : data.status === "Pending"
                      ? "/client/components/asserts/image3.png"
                      : data.status === "Approved"
                      ? "/client/components/asserts/image4.png"
                      : ""
                  }
                  alt={data.status}
        ...
        <div className="invoice-section space-y-6 mt-8">
          {/* Issued From */}
          <div>
            <div className="font-semibold uppercase tracking-wider text-xs text-neon-blue">ISSUED FROM:</div>
          ...
          {/* Issued To */}
          <div>
            <div className="font-semibold uppercase tracking-wider text-xs text-neon-blue">ISSUED TO:</div>
        ...
        <div className="invoice-section mt-8">
          <div className="font-semibold uppercase tracking-wider text-xs text-neon-blue">PROJECT DETAILS:</div>
        ...
        <div className="invoice-section mt-6">
          <div className="font-semibold uppercase tracking-wider text-xs mb-2 text-neon-blue">BILLING DETAILS:</div>
          <table className="invoice-table w-full text-sm border-t border-b">
            ...
            <tbody>
              {data.items.map((it, idx) => (
                <tr key={it.id} className="border-t hover:bg-gradient-to-r hover:from-neon-blue/5 hover:to-neon-orange/5">
        ...
        <div className="invoice-section grid grid-cols-2 gap-8 mt-8">
          <div>
            <div className="uppercase tracking-wider text-xs font-semibold mb-2 text-neon-blue">PAYMENT METHOD:</div>
          ...
          <div>
            <div className="uppercase tracking-wider text-xs font-semibold mb-2 text-neon-blue">TERM AND CONDITIONS:</div>
            <div className="h-1.5 w-28 bg-gradient-to-r from-neon-blue to-neon-orange mb-2" />
      ...
      <div className="bottom-brand-bar h-20 w-full flex items-center justify-between px-10 bg-gradient-to-r from-neon-orange to-neon-blue text-white absolute bottom-0 left-0 right-0">
        <div className="text-lg font-bold">1 st Floor, MSR Tech Park, Novel Office, Marathahalli, Bangalore, Karnataka, 560036.</div>
        <div className="text-lg font-bold">ENKONIX SOFTWARE SERVICES PVT LTD</div>
      </div>
    </div>
  );
